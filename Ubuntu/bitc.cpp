...
void parseFactor(string &line, int &pos) {
            ...
            if (i != 6) { // (expr) does nothing to expr, so no need to pop or push anything
                ...
                switch (i) {
                    case 0: // xor(ax,cx)
                         ...
                         */
                    case 1: // ls(ax,cx)
                        outFile << "\tshl ax, cl" << endl;
                        break;
                    case 2: // rs(ax, cx)
                        outFile << "\tshr ax, cl" << endl;
                        break;
                    case 3: // lr(ax, cx)
                        outFile << "\trol ax, cl" << endl;
                        break;
                    case 4: // rr(ax, cx)
                        outFile << "\tror ax, cl" << endl;
                        break;
                    case 5: // not(ax)
                        outFile << "\tnot ax" << endl;
                        break;
                    default:
                        cerr << "Weird ERROR !!!" << endl;
                }
    ...
    if (isNumOrVar) {
        ...
        if (line[startPos] == '$') {
            ...
            outFile << "\tpush " << addVariable(myNumOrVar) << " w" << endl;
        } else {
            if (!isHexadecimal(line[startPos])) {
                errOnLine = true;
                return;
            }
            outFile << "\tpush 0" << myNumOrVar << "h" << endl;
        }
    }
}
...
void parseLine(string &line) {
    ...
    for (int i = 0; i < len; ++i) {
        if (line[i] == '=') {
            ...
            ++i;
            outFile << "\tpush offset " << addVariable(myVariable) << endl;
...
int main (int argc, char* argv[]) {
    ...
    if (error) {
        cerr << "Run the code with the following command: ./bitc [input_file].bc" << endl;
        return 1;
    }
    ...
    do {
        ...
        if (line.length() > 1) {
            ...
            ++currLineNo;
            if (errOnLine) {
                cerr << "Syntax error on line " << currLineNo << endl;
                return 0;
            }
        }
    } while (inFile.good());
